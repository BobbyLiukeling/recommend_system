...
device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu") #本PC无配套GPU，使用CPU训练
...
def train(g, d, train_loader, neg_loader, epochs, g_optim, d_optim, datasetLen):
    ...
    for epo in tqdm(range(epochs)):
        ...
        print("\n times:",epo+1," 耗时:%.2fs, d_loss_sum:%.5f, g_loss:%.5f " % ((end - start), d_loss_sum, g_loss))
        start = end
        # test_model---
        item, attr = test_model.get_test_data()
        item = item.to(device)
        attr = attr.to(device)
        item_user = g(attr)
        test_model.to_valuate(item, item_user)
